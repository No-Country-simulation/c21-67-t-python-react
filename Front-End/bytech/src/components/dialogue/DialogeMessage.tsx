...
const DialogeMessage: React.FC<DialogProps> = ({
  ...
  const getIcon = () => {
    switch (type) {
      ...
      case "ERROR":
        return (
          <MdReportGmailerrorred className="h-12 w-12 text-destructive mb-4" />
        );
      case "ALERTA":
        return <MdWarning className="h-12 w-12 text-warning mb-4" />;
      default:
        return null;
    }
  };
      ...
      <AlertDialogContent className="fixed flex items-center justify-center z-50">
        <div className="rounded-lg p-10 shadow-lg justify-between border-primary border-2">
          <AlertDialogHeader className="flex flex-col items-center">
            <AlertDialogTitle className="mb-2 text-3xl text-primary">
              {type}
            </AlertDialogTitle>
            {getIcon()}
          ...
          <AlertDialogFooter className="mt-6">
            {textButtonOne && (
              <AlertDialogAction
                ...
                className={
                  styleButton1 ??
                  "hover:bg-white hover:text-secondary hover:border-secondary border-2 text-white bg-secondary"
                }
              >
                {textButtonOne}
              </AlertDialogAction>
